---
title: "Users API: Register and Manage Kantuta POS Accounts"
sidebarTitle: "Users"
...
Returns all registered user accounts as an array. Each user record includes the linked `Persona` and `Role` objects. Deactivated accounts (`estado: false`) are included in the response — filter client-side if needed.
...
Removes the user account identified by `id`. Requires a valid Bearer token.
