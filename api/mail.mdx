...
sidebarTitle: "Mail"
description: "REST endpoint for sending email notifications from Kantuta POS Backend. Supports generic messages and templated emails via Nodemailer or Resend."
---

The Mail module handles all outbound email delivery for Kantuta POS. It exposes a single public REST endpoint for sending ad-hoc messages and powers four Handlebars-rendered HTML templates that are triggered automatically by other modules — welcome confirmations, password reset codes, and post-change security alerts. Under the hood the service is wired to **Resend** for transactional delivery (configured via `RESEND_API_KEY`), with a full **Nodemailer / SMTP** transport available as a fallback through the `@nestjs-modules/mailer` integration.
...
<Note>
  The production service uses **Resend** (`RESEND_API_KEY`). The `MailerModule` SMTP transport (configured with `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_USER`, `EMAIL_PASSWORD`, and `EMAIL_SECURE`) is registered as the NestJS mailer transport but the `MailService` methods call `resend.emails.send()` directly, so Resend takes precedence at runtime.
</Note>
